...
/* ---- Framework band --------------------------------------------------- */
/* Visually brackets the KAG-framework modules (04–05) as a distinct
   "special section" set apart from the foundations that precede it. */
    ...
    >
      ...
      >
        ...
        <span style={{ fontFamily: SANS, fontSize: 13, color: C.muted, lineHeight: 1.4 }}>
          The core contribution — everything above is the ground it stands on.
        </span>
      </div>
      {children}
    </div>
...
/* ---- Table ------------------------------------------------------------ */
export function Table({ head, rows, highlightLast }) {
  return (
    <div style={{ overflowX: 'auto', margin: '16px 0' }}>
      ...
      >
        ...
        <tbody>
          {rows.map((r, ri) => {
            const isKag = highlightLast && ri === rows.length - 1;
            return (
              <tr key={ri} style={{ background: isKag ? `${C.green}0d` : 'transparent' }}>
                {r.map((cell, ci) => (
                  <td
                    key={ci}
                    style={{
                      textAlign: ci === 0 ? 'left' : 'right',
                      color: isKag ? C.green : ci === 0 ? C.text : C.muted,
                      fontWeight: isKag && ci > 0 ? 700 : 400,
                      padding: '7px 12px',
                      borderBottom: `1px solid ${C.line}`,
                      whiteSpace: 'nowrap',
                    }}
...
export function Ref({ authors, title, venue, href }) {
  return (
    <a
      href={href}
      target="_blank"
